...
class TSVModel(AbstractModel):
    def __init__(self, args):
        ...
        self.cats = self._parseTsv(dir, 'categories.tsv', ['id', 'label'])

        Logger.debug(__name__, 'Load messages data')
        self.msgs = self._parseTsv(dir, 'messages.tsv', ['id', 'cat_id', 'label', 'text'])
        ...
        self.loading_msgs = self._parseTsv(dir, 'loading.tsv', ['id', 'message'], default)

        Logger.debug(__name__, 'Load custom log messages data')
        self.log_msgs = self._parseTsv(dir, 'log.tsv', ['id', 'message'], OrderedDict())
    ...
    def msgs(self, cat_id = None, msg_id = None):
        ...
        if msg_id is not None:
            try:
                ...
                raise KeyError('No matching message for id "%s" in %s' % (msg_id, cat_id_str))

        return self.msgs

    def _parseTsv(self, dir, file_name, field_names, default=None):
        """Parse a tab-separated values file into an OrderedDict. Must include 
        an id column (specify location with the field_names argument).

        Arguments:
            dir {str} -- Directory to load file from
            file_name {str} -- File name to parse in the configured directory
